...
describe('Festival-Moon Ledger', () => {
  ...
  describe('Transaction Creation', () => {
    ...
    it('should create a valid covenant wage transaction', async () => {
      const transaction = await ledger.createTransaction(
        fromAccount.id,
        toAccount.id,
        120, // $120 for 8 hours = $15/hour (meets fair wage)
        TransactionType.COVENANT_WAGE,
        'Web development work - 8 hours',
        'Fair wage for legitimate development work'
      );
    ...
    it('should create a dignity preservation transaction', async () => {
      const transaction = await ledger.createTransaction(
        ...
        toAccount.id,
        50,
        TransactionType.DIGNITY_PRESERVATION,
  ...
  describe('Transaction Processing', () => {
    ...
    beforeEach(async () => {
      ...
      transaction = await ledger.createTransaction(
        fromAccount.id,
        toAccount.id,
        200,
        TransactionType.COVENANT_WAGE,
        'Development work - 10 hours',
        'Fair wage payment'
      );
    });
    ...
    it('should not process pending transaction', async () => {
      ...
      const pendingTransaction = await ledger.createTransaction(
        fromAccount.id,
        toAccount.id,
        30, // Below fair wage
        TransactionType.COVENANT_WAGE,
        'Development work - 8 hours' // Would be below fair wage
      );
    ...
    it('should not process covenant violation transaction', async () => {
      ...
      const violationTransaction = await ledger.createTransaction(
        fromAccount.id,
        toAccount.id,
        50, // Below fair wage for development work
        TransactionType.COVENANT_WAGE,
        'Development work' // Would be flagged as below fair wage
      );
  ...
  describe('Transaction History', () => {
    ...
    it('should retrieve transaction history for account', async () => {
      ...
      await ledger.createTransaction(
        account1.id, account2.id, 100, 
        TransactionType.COVENANT_WAGE, 'Transaction 1'
      );
      await ledger.createTransaction(
        account1.id, account2.id, 150, 
        TransactionType.GIFT_ECONOMY, 'Transaction 2'
      );
      
      // Add balance to account2 for the third transaction
      await ledger.addBalance(account2.id, 500);
      await ledger.createTransaction(
        account2.id, account1.id, 75, 
        TransactionType.DIGNITY_PRESERVATION, 'Transaction 3'
      );
    ...
    it('should limit transaction history results', async () => {
      ...
      for (let i = 0; i < 10; i++) {
        await ledger.createTransaction(
          account1.id, account2.id, 10, 
          TransactionType.COVENANT_WAGE, `Transaction ${i}`
        );
      }
  ...
  describe('Covenant Compliance', () => {
    ...
    it('should verify fair wage for development work', async () => {
      const transaction = await ledger.createTransaction(
        fromAccount.id,
        toAccount.id,
        120, // $120 for 8 hours = $15/hour (fair wage)
        TransactionType.COVENANT_WAGE,
        'Development work - 8 hours'
      );
    ...
    it('should flag below fair wage transactions', async () => {
      const transaction = await ledger.createTransaction(
        fromAccount.id,
        toAccount.id,
        50, // $50 for estimated development work (below fair wage)
        TransactionType.COVENANT_WAGE,
        'Development work'
      );
    ...
    it('should approve gift economy transactions without wage verification', async () => {
      const transaction = await ledger.createTransaction(
        ...
        toAccount.id,
        25,
        TransactionType.GIFT_ECONOMY,
        'Community support gift'
      );
  ...
  describe('Financial Reporting', () => {
    ...
    it('should generate financial summary for account', async () => {
      ...
      const transaction1 = await ledger.createTransaction(
        otherAccount.id, account.id, 200, 
        TransactionType.COVENANT_WAGE, 'Income transaction'
      );
      await ledger.processTransaction(transaction1.id);

      const transaction2 = await ledger.createTransaction(
        account.id, otherAccount.id, 100, 
        TransactionType.STEWARDSHIP_INVESTMENT, 'Expense transaction'
      );
    ...
    it('should throw error for non-existent account', async () => {
      await expect(
        ledger.getFinancialSummary('non_existent')
  ...
  describe('Dignity Impact Calculation', () => {
    ...
    it('should calculate higher dignity impact for dignity preservation', async () => {
      const dignityTransaction = await ledger.createTransaction(
        fromAccount.id, toAccount.id, 100,
        TransactionType.DIGNITY_PRESERVATION, 'Housing assistance'
      );

      const wageTransaction = await ledger.createTransaction(
        fromAccount.id, toAccount.id, 100,
        TransactionType.COVENANT_WAGE, 'Regular work payment'
      );

      expect(dignityTransaction.dignityImpactScore).toBeGreaterThan(
        wageTransaction.dignityImpactScore
      );
    });

    it('should scale dignity impact with transaction amount', async () => {
      const smallTransaction = await ledger.createTransaction(
        fromAccount.id, toAccount.id, 50,
        TransactionType.GIFT_ECONOMY, 'Small gift'
      );

      const largeTransaction = await ledger.createTransaction(
        fromAccount.id, toAccount.id, 500,
        TransactionType.GIFT_ECONOMY, 'Large gift'
      );

      expect(largeTransaction.dignityImpactScore).toBeGreaterThan(
        smallTransaction.dignityImpactScore
      );
    });
  });
});