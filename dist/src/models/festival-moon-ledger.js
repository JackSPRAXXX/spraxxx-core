...
import { TransactionType, TransactionStatus } from './ledger-types.js';
export class FestivalMoonLedger {
    ...
    // Account Management
    async createAccount(stewardId, type, organizationId) {
    ...
    async getAccountsByStward(stewardId) {
    ...
    async createTransaction(fromAccountId, toAccountId, amount, type, description, covenantJustification) {
        if (amount <= 0) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `, id, fromAccountId, toAccountId, amount, type, TransactionStatus.PENDING, description, covenantJustification, dignityImpactScore, now);
        // Perform covenant compliance check
        await this.performCovenantCompliance(id);
        return this.getTransaction(id);
    }
    calculateDignityImpact(type, amount) {
        const baseScore = {
    ...
    // Covenant Compliance
    async performCovenantCompliance(transactionId) {
        const transaction = await this.getTransaction(transactionId);
        if (!transaction)
            return;
      ...
      VALUES (?, ?, ?, ?, ?, ?)
    `, complianceId, transactionId, isCompliant, fairWageVerified, transaction.dignityImpactScore, JSON.stringify(notes));
    ...
    verifyFairWage(amount, description) {
    ...
    // Financial Reporting
    async getFinancialSummary(accountId) {
