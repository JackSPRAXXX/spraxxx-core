...
async function simulateFestivalMoonCovenant() {
  ...
  const enterprise = await ledger.createAccount(
    'alice_steward', 
  ...
  const wageTransaction = await ledger.createTransaction(
    alice.id,
    bob.id,
    240, // $240 for 16 hours = $15/hour (fair wage)
  ...
  const dignityTransaction = await ledger.createTransaction(
    ...
    'Emergency housing assistance for covenant member',
    'Preserving human dignity through community support'
  );
  ...
  const giftTransaction = await ledger.createTransaction(
    bob.id,
    treasury.id,
    100,
    TransactionType.GIFT_ECONOMY,
    'Thanksgiving gift to community treasury'
  );
  ...
  const ignitionTransaction = await ledger.createTransaction(
    enterprise.id,
    treasury.id,
    250, // $50 x 5 keys
    TransactionType.IGNITION_KEY_PAYMENT,
    'Enterprise ignition keys - 5 developers',
    'Payment for SPRAXXX access and dignity protection'
  );
  ...
  const investmentTransaction = await ledger.createTransaction(
    alice.id,
    treasury.id,
    300,
    TransactionType.STEWARDSHIP_INVESTMENT,
    'Investment in covenant server infrastructure'
  );
  ...
  try {
    const violationTransaction = await ledger.createTransaction(
      alice.id,
      charlie.id,
      30, // $30 for development work (below fair wage)
      TransactionType.COVENANT_WAGE,
      'Development work - 8 hours' // Would be $3.75/hour
    );
